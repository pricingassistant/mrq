...
def test_cancel_by_path(worker, p_query):
  ...
  assert worker.mongodb_logs.tests_inserts.count() == len(job_ids) - 1 - expected_action_jobs
  ...
  assert worker.mongodb_jobs.mrq_jobs.find({"status": "queued"}).count() == expected_action_jobs

  assert Queue("default").size() + Queue("q1").size() + Queue("q2").size() == expected_action_jobs
