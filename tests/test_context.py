...
def test_context_metric_success(worker):
  ...
  metrics = json.loads(worker.send_task("tests.tasks.general.GetMetrics", {}))

  assert metrics.get("queues.default.dequeued") == 3  # GetMetrics is also a task!
...
def test_context_metric_queue(worker):
  ...
  worker.send_task("tests.tasks.general.SendTask", {"path": "tests.tasks.general.Add", "params": {"a": 41, "b": 1}})
...
def test_context_metric_failed(worker):
  ...
  worker.send_task("tests.tasks.general.RaiseException", {}, accept_statuses=["failed"])

  metrics = json.loads(worker.send_task("tests.tasks.general.GetMetrics", {}))

  assert metrics.get("queues.default.dequeued") == 2  # GetMetrics is also a task!
